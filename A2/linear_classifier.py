...
def svm_loss_vectorized(
    ...
    _, num_class = W.shape
    margin = X.matmul(W) - torch.diag(X.matmul(W[:, y])).view(num_sample, -1) + 1
    margin[torch.arange(num_sample), y] = 0
    margin[margin < 0] = 0
    loss = torch.sum(margin) / num_sample + reg * torch.sum(W.pow(2))
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    #############################################################################
    # Replace "pass" statement with your code
    dW1 = torch.sum(X, dim=0, keepdim=True).T
    dW2 = (-(num_class) * X).T
    dW3 = torch.zeros(num_sample, num_class).to(device=W.device, dtype=W.dtype)
    dW3[torch.arange(num_sample), y] = 1
    dW = (dW1 + dW2.matmul(dW3)) / num_sample
    dW += 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    score = X.matmul(W)
    _, y_pred = torch.max(score, dim=1)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
